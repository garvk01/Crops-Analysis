...
const TYPE_CFG = {
  success:  { label:'Great',    dot:'#5ec962', bg:'rgba(94,201,98,.10)',   border:'rgba(94,201,98,.22)',   text:'#8fd090' },
...
function resolveType(insight) {
  const title = (insight.title || '').toLowerCase()
  const msg   = (insight.message || '').toLowerCase()
  if (insight.type && insight.type !== 'info') return insight.type
  if (title.includes('critical') || title.includes('severe')  || title.includes('failed'))                                          return 'critical'
  if (title.includes('warning')  || title.includes('delayed') || title.includes('low') || title.includes('stress') || msg.includes('below threshold')) return 'warning'
  if (title.includes('strong')   || title.includes('good')    || title.includes('healthy') || title.includes('optimal'))            return 'success'
  return 'info'
}
...
function InsightCard({ insight }) {
  const cfg      = TYPE_CFG[resolveType(insight)] || TYPE_CFG.info
    ...
    >
      <div className={styles.cardHeader}>
        ...
        <svg
          ...
          viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" width="14" height="14"
        >
          ...
          {insight.value !== undefined && (
            <div className={styles.values}>
              <span className={styles.valueChip}>
                Measured: <strong>{typeof insight.value === 'number' ? insight.value.toFixed(insight.value > 1 ? 1 : 3) : insight.value}</strong>
              </span>
              {insight.threshold !== undefined && (
                <span className={styles.valueChip}>
                  Threshold: <strong>{typeof insight.threshold === 'number' ? insight.threshold.toFixed(insight.threshold > 1 ? 0 : 3) : insight.threshold}</strong>
                </span>
              )}
            </div>
          )}
...
export default function InsightsPanel({ cropDataId, preloadedInsights }) {
  ...
  useEffect(() => { if (!preloadedInsights && cropDataId) loadInsights() }, [cropDataId])

  const loadInsights = async () => {
    try {
      setLoading(true)
      const r = await api.get(`/insights/${cropDataId}`)
  ...
  const regenerate = async (e) => {
    ...
    setRegen(true)
    try {
      const r = await api.post(`/insights/${cropDataId}/regenerate`)
      setInsights(r.data.insights || [])
    } catch { /* graceful */ } finally { setRegen(false) }
  }

  const counts = { success: 0, critical: 0, warning: 0, info: 0 }
  insights.forEach(i => {
    const t = resolveType(i)
    counts[t] = (counts[t] || 0) + 1
  })

  if (loading) return (
    <div className={styles.loading}>
      <div className={styles.spinner}/>Generating insights...
    </div>
  )
      ...
      <div className={styles.panelHeader}>
        <div>
          ...
          <div className={styles.panelSub}>
            {counts.success  > 0 && <span style={{ color:'#8fd090' }}>{counts.success} good · </span>}
        ...
        {cropDataId && (
          <button className={styles.regenBtn} onClick={regenerate} disabled={regen}>
            {regen ? '...' : '↻ Refresh'}
          </button>
        )}
      </div>
      ...
      {insights.length === 0
        ? (
          <div className={styles.empty}>
            ...
            <div className={styles.emptyText}>All metrics within normal range</div>
          </div>
        )
        : (
